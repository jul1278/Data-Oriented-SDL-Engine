...
public:
	...
	std::map<int, BaseComponent*> operator[](ComponentType componentType)
	{
		if (this->typeIdMap.find(componentType) != this->typeIdMap.end()) {
			...
			for (int id : ids) {
				components.insert(std::pair<int, BaseComponent*>(id, componentMap[id])); 
			}
			return components; 
		}
	}

	void Insert(ComponentType componentType, BaseComponent* component)
	{	
		...
		if (this->typeIdMap.find(componentType) != this->typeIdMap.end()) {
			...
			this->componentMap.insert(std::pair<int, BaseComponent*>(component->id, component)); 
		} else {
